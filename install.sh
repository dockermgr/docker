...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/docker"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="docker"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/docker"
export DOCKERMGR_CONFIG_DIR="${DOCKERMGR_CONFIG_DIR:-$HOME/.config/myscripts/$SCRIPTS_PREFIX}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the mountpoint directory
export APPDIR="$HOME/.local/share/srv/docker/docker"
...
__printf_spacing_color() { printf "%b%-${2:?Please set spacing number}s%s%b\n" "$(tput setaf "${1:?Please set color number}" 2>/dev/null)" "${3:?Please set left content}" "${4:-Empty Variable}" "$(tput sgr0 2>/dev/null)"; }
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set custom script - saves to /config/env/docker.script.sh
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-docker}}"
...
if [ -n "$DOCKER_USERNAME" ]; then
  CONTAINER_USER_NAME="$DOCKER_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${DOCKER_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
